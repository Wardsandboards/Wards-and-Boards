...
function AuthorProfile({ a, onBack, isReal = false }: { a: Author; onBack: () => void; isReal?: boolean }) {
    ...
    <div>
      ...
      <div className="profile-head">
        ...
        <div style={{ flex: 1, minWidth: 240 }}>
          <h2 className="h2" style={{ margin: 0 }}>{a.name} {a.creds && <span className="ab-cred" style={{ fontSize: '1rem' }}>{a.creds}</span>}</h2>
          <div className="profile-role">{a.role}{a.institution ? ' · ' + a.institution : ''}</div>
          {a.badges.length > 0 && <div className="badge-row">{a.badges.map((b, i) => <AuBadge key={i} icon={b[0]} label={b[1]} />)}</div>}
          {a.bio && <p style={{ color: 'var(--mid)', fontSize: '0.9rem', margin: '4px 0 10px' }}>{a.bio}</p>}
          <div className="link-row">
            {a.institution && <span className="link-chip">🏛 {a.institution}</span>}
            {a.orcid && <span className="link-chip">ORCID {a.orcid}</span>}
            {!isReal && <span className="link-chip">Doximity</span>}
          </div>
          {!isReal && (
      ...
      <div className="stat-grid">
        ...
        {a.joined && <div className="stat-box"><div className="sb-n">{a.joined}</div><div className="sb-l">Contributing since</div></div>}
      </div>
      ...
      {a.qs.length === 0 ? <p style={{ color: 'var(--dim)' }}>No published questions yet.</p> : a.qs.map((q, i) => (
        <div className="aqlist-row" key={i}>
          <div><div className="aqlist-title">{q[0]}</div><div className="aqlist-meta">{q[1]}{q[3] ? ' · ' + q[3] : ''} · <span className="verify-mini">✓ reviewed</span></div></div></div>
      ))}
    </div>
  )
}

export function AuthorsView({ sel, setSel, authors = COMMUNITY_AUTHORS, isReal = false }: { sel: Author | null; setSel: (a: Author | null) => void; authors?: Author[]; isReal?: boolean }) {
  useEffect(() => { window.scrollTo({ top: 0, left: 0, behavior: 'auto' }) }, [sel])
  const sorted = [...authors].sort((x, y) => y.published - x.published)
  if (sel) return (<section className="section" style={{ paddingTop: 34 }}><div className="wrap" style={{ maxWidth: 760 }}><AuthorProfile a={sel} onBack={() => setSel(null)} isReal={isReal} /></div></section>)
  return (
