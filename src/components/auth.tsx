...
export function ContributorApplication({ name, appStatus, onApply }: { name: string; appStatus: string; onApply: (form: { training: string; institution: string; npi: string }) => void }) {
  ...
  if (appStatus === 'pending')
    return (
      <div className="notice pending"><div className="kicker">Application received</div>
        <h2 className="h2">Thanks, {name}</h2>
  ...
  return (
    <div className="qblock" style={{ maxWidth: 640 }}>
      ...
      {appStatus === 'denied' && <div className="feedback bad" style={{ marginBottom: 12 }}>A previous application was not approved. You can update your details and resubmit.</div>}
...
export function AdminQueue({ pending, onDecide }: { pending: PendingApp[]; onDecide: (id: string, decision: string) => void }) {
  return (
    <div>
      ...
      {pending.length === 0 ? <div className="inprog"><p>No pending applications. (Sign in as a new learner, apply from the Contribute tab, then come back here as admin to approve.)</p></div> :
        pending.map((u) => (
          <div className="appq" key={u.id}>
            <div style={{ fontWeight: 800, color: 'var(--ink)' }}>{u.name} <span className="userchip">· {u.email}</span></div>
            <div style={{ fontSize: '0.88rem', color: 'var(--mid)', margin: '6px 0' }}>{u.training} · {u.institution} · verify: {u.npi}</div>
            <div className="case-actions"><button className="submit-btn" style={{ marginTop: 0, background: 'var(--good)' }} onClick={() => onDecide(u.id, 'approve')}>Approve</button>
              <button className="ghost-btn" onClick={() => onDecide(u.id, 'deny')}>Deny</button></div>
          </div>
        ))}
    </div>
  )
